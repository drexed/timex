...
You already have an **`IO.select`** loop juggling real sockets. **Wakeup** hands you an extra pipe—backed by a **`CancellationToken`**—so the select set can also react when **your deadline fires**, not only when bytes arrive.
...
Each `Wakeup` is **single-use**: always `close` in an `ensure` block, and build a fresh instance for the next operation. The pipe and watcher thread leak if you forget.
...
A mobile client long-polls `/inbox/wait` for up to 25 s. You subscribe to a Redis pub/sub channel and want `select` to wake on **either** a new message **or** the deadline—never both clients sitting on dead sockets:
...
No per-request thread killing, no `Timeout.timeout` wrapping a Redis read—the kernel’s own `select` handles the wait and the doorbell rings on schedule.
